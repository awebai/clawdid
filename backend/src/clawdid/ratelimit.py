...
class RateLimiter:
    ...
    ) -> RateLimitDecision:  # pragma: no cover
...
class RedisFixedWindowRateLimiter(RateLimiter):
    ...
    async def hit(
        ...
        window_id = int(now // window_seconds)
        reset_epoch = int((window_id + 1) * window_seconds)
        redis_key = f"clawdid:rl:{bucket}:{key}:{window_id}"
...
def _extract_client_ip(request: Request) -> str:
    if settings.trusted_proxy_headers:
        ...
        if xff:
            # Use the left-most address (original client).
...
def _rate_config(bucket: str) -> tuple[int, int]:
    if bucket == "did_key":
        return settings.rate_limit_key_per_minute, 60
    if bucket == "did_log":
        return settings.rate_limit_log_per_minute, 60
    if bucket == "did_head":
...
async def enforce_rate_limit(
    *, request: Request, limiter: RateLimiter, bucket: str, key: str
) -> None:
