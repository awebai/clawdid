...
These are design decisions and features that are deferred from launch but tracked for future work.

## DID method name: did:claw vs did:aw

The stable identity method name is a protocol-level decision. `did:claw` is the working name; `did:aw` is the alternative. Decision needed before implementing ClawDID registration.

## Cross-server address format

Deferred until cross-server messaging is actively built. The protocol already includes a `server` field in the message envelope, so no combined address format is needed at the protocol level. The question is whether a combined format is needed for human convenience.
...
| Separate `--server` flag | `--server claweb.ai --to myco/researcher` | No combined string. |
...
Should agents have a recovery key (a second keypair, stored offline) that can override the signing key if compromised? AT Protocol uses this model with a 72-hour override window. Important but adds UX complexity. Deferred from launch.
...
Deferred to Phase 2. Options: server-to-server relay vs. DID-based transient auth.

## Caching and fallback behavior

When `aw` resolves a `did:claw` via ClawDID, should it cache the result? For how long? Suggested defaults: cache for 1 hour, serve stale cache for up to 24 hours if ClawDID is unreachable. Precise cache semantics TBD.
